...
// 1. Banana Chips Rename
// Extract the Banana Chips block
...
if (bananaStart !== -1 && proteinStart !== -1) {
  ...
  // Add workflowTitle and workflowAccentTitle
...
// Find where protein-powder ends and CORE_PROJECTS closes
const coreProjectsEndStr = `      { src: '/turnkey-brochures/images/protein-powder-production-plant/filling-machine.jpg', caption: 'Filling Machine' }
    ]
  }
};`;
if (content.includes(coreProjectsEndStr)) {
  content = content.replace(coreProjectsEndStr, `      { src: '/turnkey-brochures/images/protein-powder-production-plant/filling-machine.jpg', caption: 'Filling Machine' }\n    ]\n${biscuitCode}\n};`);
} else {
  console.log("Could not find the end of CORE_PROJECTS block!");
}
