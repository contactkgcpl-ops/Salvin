...
// The file might be corrupted, let's just write the correct missing parts.
...
*/
// It deleted the closing `}` of the last FAQ, the `]`, the `GALLERY_IMAGES`, and the `export default function FaceCreamManufacturingDetailPage() {` as well as the states!

// Let's just fix it by finding the last FAQ string.
const faqEndString = 'answer: "Yes, we provide a complete turnkey solution. Our expert engineers will come to your factory, install all the machines, start the production, and hand over a running plant to you."';
...
// Replace everything from faqEndString to `// Scroll to top only on initial page entry`
const regex = new RegExp(faqEndString.replace(/[.*+?^$\\{\\}()|[\\]\\\\]/g, '\\\\$&') + '[\\\\s\\\\S]*?// Scroll to top only on initial page entry');

jsxCode = jsxCode.replace(regex, faqEndString + fixContent + '\\n  // Scroll to top only on initial page entry');

fs.writeFileSync(jsxPath, jsxCode);
console.log("Fixed Face Cream JSX!");
