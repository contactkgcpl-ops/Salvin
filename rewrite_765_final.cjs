...
pages.forEach(p => {
  let processJSX = p.processSteps.map((step) => {
    let icon = svgs[step.id - 1] || svgs[0];
...
'              <p><strong>Salvin Industries\' ' + p.title + '</strong> is an advanced industrial turnkey solution engineered to extract premium-grade output with unmatched efficiency. Our comprehensive process guarantees a seamless, hygienic operation that maximizes product yield.</p>\n' +
'              <p>' + p.desc + '</p>\n' +
...
console.log("Successfully generated SEO rich detail pages with exactly 7 workflow steps, 6 machines, and 5 FAQs!");
